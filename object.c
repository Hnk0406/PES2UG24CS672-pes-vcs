...
if (!full) {printf("error1"); return -1;}
...
memcpy(full + header_len + 1, data, len);
    ...
    if (fd < 0) { free(full); printf("error2");return -1; }
 
    ssize_t written = write(fd, full, full_len);
    free(full);
    if (written < 0 || (size_t)written != full_len) {
        close(fd); unlink(tmp_path);printf("error3"); return -1;
    }
 
    if (fsync(fd) != 0) { close(fd); unlink(tmp_path);printf("error4"); return -1; }
    close(fd);
 
    if (rename(tmp_path, final_path) != 0) {
        unlink(tmp_path); printf("error5");return -1;
    }
