...
int index_load(Index *index) {
    ...
    if (!f) return 0; // No index file yet — empty index is OK
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf(line, "%o %64s %llu %u %511s",
                   &e->mode, hex, (unsigned long long *)&mtime, &size, path) != 5) {
            continue;
        }
...
int index_save(const Index *index) {
    // Sort a heap copy before writing (Index too large for stack)
...
int index_add(Index *index, const char *path) {
    ...
    uint8_t *content = malloc((size_t)file_size + 1);
    ...
    ObjectID hash;
    if (object_write(OBJ_BLOB, content, (size_t)file_size, &hash) != 0) {
